...
import { FaRegUser, FaEye, FaEyeSlash, FaGoogle, FaFacebook } from "react-icons/fa";
  ...
  return (
    ...
    <div className=' h-auto w-80 p-8 rounded-md border border-solid border-opacity-10 shadow-xl' style={{backdropFilter: "blur(50px)"}}>
        ...
        <form className='w-full pt-8' onSubmit={handleSubmit}>
          ...
          onChange={(e) => setIsChecked(e.target.checked)}/><label htmlFor='rememberMe' className='text-sm'>Remember Me</label>
            </div>
            </div>
            <div className='flex flex-col items-center pt-5'>
            <button type='submit' className='bg-gray-800 hover:bg-black text-white rounded-md p-2 w-full' onClick={RevokeLogin}>LogIn</button>
            <a href='#' className='text-blue-500 hover:text-red-700 underline'><span className='text-sm'>Forgot Password?</span></a>
            </div>
